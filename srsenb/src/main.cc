...
void parse_args(all_args_t* args, int argc, char* argv[])
{
  ...
  common.add_options()
    ...
    ("scheduler.tpc_min_snr_db", bpo::value<float>(&args->stack.mac.sched.tpc_min_snr_dB)->default_value(-4.0), "Minimum UL SNR sample (in dB) accepted by the UL power control loop. Samples below are treated as no-transmission.")
